...
### Fix Now (Critical + High = 15 findings)
...
| DLC-001 | High | dlc-audit | Add code-execution warning; add timeouts |
| DLC-004 | High | dlc-audit | Align grade boundary scales |
| DLC-008 | High | dlc-audit | Add `timeout 30` to network-dependent commands |
| DLC-010 | High | dlc-audit | Add `quick` mode; document token consumption |
| DOC-003 | High | docs | Rename to "AI-DLC documentation standard" |
| MH-003 | High | motherhen | Synchronize foundation doc search paths with dlc-audit |
| PROD-002 | High | prodstatus | Add confidentiality header; externalize resource names (PROD-003 redacted) |
| PROD-007 | High | prodstatus | Replace sequential API calls with batch `list-functions` |
...
| TK-005 | High | ticky | Align key format with naming convention |
| TK-009 | High | ticky | Add idempotency check to clean mode |
| TK-014 | High | ticky | Document rollback procedure or add retract mode |

### Fix Soon (Medium = 34 findings)

| ID | Skill | Action |
|----|-------|--------|
| DLC-002 | dlc-audit | Add prompt injection awareness note |
| DLC-005 | dlc-audit | Add calibration examples to rubrics |
| DLC-006 | dlc-audit | Detect installed skills before recommending |
| DLC-009 | dlc-audit | Add directory exclusions to find command |
| DLC-011 | dlc-audit | Add confirmation to `full` mode |
| DLC-012 | dlc-audit | Add sample output section |
| DLC-013 | dlc-audit | Show met/missed criteria in dashboard |
| DOC-001 | docs | Add secret sanitization instruction |
| DOC-004 | docs | Replace absolute line count with proportional guidance |
| DOC-006 | docs | Add git-safety pre-flight check |
| DOC-007 | docs | Scope file reads by action type |
| DOC-008 | docs | Define merge behavior for existing docs |
| MH-001 | motherhen | Replace vitest --run with list-only; add code-exec warning |
| MH-004 | motherhen | Add rationale for staleness thresholds |
| MH-006 | motherhen | Fix .NET project detection to be recursive |
| MH-009 | motherhen | Add PASS/WARN/FAIL to 0-10 mapping note |
| PM-001 | pm | Pin max git range; add commit message safety note |
| PM-003 | pm | Define MEMORY.md path and format, or remove references |
| PM-005 | pm | Replace pytest --co with file-based counting |
| PM-008 | pm | Add first-run bootstrap path |
| PROD-003 | prodstatus | ~~Redact real username/account in examples~~ **RESOLVED** |
| PROD-004 | prodstatus | Use clearly fake example values |
| PROD-005 | prodstatus | Add day-1-of-month guard for cost command |
| PROD-008 | prodstatus | Add per-command timeouts |
| PROD-009 | prodstatus | Fix canary invoke output routing |
| PROD-010 | prodstatus | Add cost API result caching |
| PROD-012 | prodstatus | Add platform compatibility note |
| TK-004 | ticky | Default clean to current repo only |
| TK-006 | ticky | Define status/ado_state mapping |
| TK-007 | ticky | Clarify draft vs legacy create trigger |
| TK-010 | ticky | Make file operations atomic |
| TK-011 | ticky | Validate ADO response before updating local state |
| TK-012 | ticky | Add rate limiting to update --all |
| TK-015 | ticky | Specify slug generation rules completely |
| TK-016 | ticky | Add dry-run to draft mode |

### Defer (Low = 22 findings)

DLC-003, DLC-007, DLC-014, DOC-002, DOC-005, DOC-009, MH-002, MH-005, MH-007, MH-008, MH-010, PM-002, PM-004, PM-006, PM-007, PM-009, PROD-006, PROD-011, PROD-013, TK-008, TK-013
